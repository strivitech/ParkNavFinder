...
using User.Location.AnalyticsService.Configurations;
using User.Location.AnalyticsService.Database;
using User.Location.AnalyticsService.Services;
...
builder.AddNpgsqlDbContext<ParkingDbContext>("UserLocationAnalyticsDb");
...
builder.Services.AddHangfire(cong => cong
    ...
    .UsePostgreSqlStorage(conf =>
        conf.UseNpgsqlConnection(builder.Configuration.GetConnectionString("UserLocationAnalyticsDb"))));
